...
function mudacor(evento) {
  ...
  for (let i =0; i < dias.length; i += 1) {
    if(dias[i].style.color === 'red') {
    dias[i].style.color = " #777"; 
    } else {
    dias[i].style.color = "red";
    } 
  }
}
...
function criarbotao (nomeBotao, pai, nomeId) {
  let butao = document.createElement('button');
  butao.textContent = nomeBotao;
  butao.id = nomeId;
  pai.appendChild(butao);
}
criarbotao('Sexta-feira',document.querySelector('.buttons-container'),'btn-friday');
